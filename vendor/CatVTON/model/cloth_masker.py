...
class AutoMasker:
    def __init__(
        ...
        np.random.seed(0)
        torch.manual_seed(0)
        # catvton-no-cuda-manual-seed — torch.cuda.manual_seed raises without CUDA build.
        if torch.cuda.is_available():
            torch.cuda.manual_seed(0)
        _mps = getattr(torch.backends, "mps", None)
        if _mps is not None and _mps.is_available() and hasattr(torch, "mps") and hasattr(torch.mps, "manual_seed"):
            torch.mps.manual_seed(0)

        # catvton-automask-cpu-on-mps — SCHP / parsing use adaptive pooling; MPS errors (pytorch#96056). Mask path stays fast on CPU.
